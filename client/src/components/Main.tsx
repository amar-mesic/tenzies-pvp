...
export default class Main extends React.Component<MainProps, MainState> {
    ...
    handleFreeze(e: SyntheticEvent, id: string) {
        ...
        this.setState((prevState: MainState) => {
            const newDieStates = prevState.diceBoard.dieStates.map(
                dieState => id !== dieState.id ? dieState : { 
                    ...dieState, 
    ...
    checkDone() {
        const completed = this.state.diceBoard.dieStates.every(dieState => dieState.isFrozen 
            && this.state.diceBoard.dieStates[0].value === dieState.value)
        
        if (completed) this.setState(prevState => ({
            ...prevState,
            diceBoard: {
                ...prevState.diceBoard,
                completed: true
    ...
    handleRoll() {
        this.setState((prevState: MainState) => ({
            ...
            diceBoard: {
                ...prevState.diceBoard,
                dieStates: prevState.diceBoard.dieStates.map(
                    dieState => dieState.isFrozen ? dieState : { 
                        ...dieState, 
            ...
            console.log(`button pressed. Dice rolling: ${this.state.diceBoard.rolling}`)
        })
    }
    ...
    handleAnimationEnd() {
        this.setState(prevState => ({
            ...
            console.log(`animation ended. Dice rolling: ${this.state.diceBoard.rolling}`)
        })
    }
    ...
    render() {
        const {dieStates: myDieStates, completed: myGameFinished, rolling } = this.state.diceBoard
        const {dieStates: oppDieStates, completed: oppGameFinished} = this.state.oppState
        const {oppReady} = this.state

        let gameButton = 
            (<button 
                className={`cool-button drop-shadow text-white ${oppReady ? '' : 'opacity-50 cursor-not-allowed'}`} 
                onClick={oppReady ? (myGameFinished || oppGameFinished ? this.reset : this.handleRoll) : () => {}}
                >
        ...
        return(
            <div className="main">
                ...
                <div className="dice-boards">
                    ...
                    <DiceBoard ready={oppReady} opponent={false} rolling={rolling} dieStates={myDieStates} handleFreeze={this.handleFreeze} handleAnimationEnd={this.handleAnimationEnd}/>
                    {oppReady 
                    ? <DiceBoard ready={oppReady} opponent={true} rolling={false} dieStates={oppDieStates} handleFreeze={this.handleFreeze} handleAnimationEnd={this.handleAnimationEnd}/> 
