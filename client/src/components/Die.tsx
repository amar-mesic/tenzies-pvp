...
export default function Die(props: MyProps) {
    ...
    const faces = new Array<JSX.Element>(6)
        ...
        .map((_, index) => {
            ...
            return <div key={nanoid()} id={`face-${index + 1}`} className={`cool-button die-face`} onClick={handleClick}>{faceValue}</div>
        })


    return(
        <div className={`cube ${isFrozen ? 'frozen' : ''}`} onAnimationEnd={handleAnimationEnd} data-rolling={props.rolling && !isFrozen ? 'true' : 'false'}>
            {faces}
        </div>
    )
}